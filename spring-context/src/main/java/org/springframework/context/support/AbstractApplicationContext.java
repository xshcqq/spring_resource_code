...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 刷新前初始化事件发布器List还有部分属性的加载
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			// 这里会判断能否刷新，并且返回一个BeanFactory
			// 看此时的Application是不是AnnotationWebConfigApplicationContext；否者不支持重复刷新
			// 如果支持刷新则执行Bean的销毁，然后重新生成一个BeanFactory
	...
	protected void prepareRefresh() {
		...
		// 比如子类重写initPropertySources()可以把ServletContext中的参数对设置到Environment
		initPropertySources();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 也就是为什么我们项目中可以直接用@Autowired可以直接注入下面这几个Bean了；Spring启动的时候就注册了
		...
		// ApplicationListenerDetector（也是后置处理器）负责把ApplicantsListener类型的Bean注册到ApplicationContext中
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		// 重点 (一下两种后置处理器执行时都按照这样的顺序：1.）实现了PriorityOrdered 2.）实现了Ordered 3.）普通的)
		// 1.执行扫描出来的普通BeanFactoryPostProcessor
		// 2.执行扫描出来的BeanDefinitionRegistryPostProcessor
